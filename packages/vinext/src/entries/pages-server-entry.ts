/**
 * Pages Router SSR server entry generator.
 *
 * Extracted from index.ts — generates the virtual SSR server entry
 * module used as the entry point for `vite build --ssr`.
 */
...
export async function generateServerEntry(pagesDir: string, nextConfig: ResolvedNextConfig | undefined, middlewarePath: string | null, fileMatcher: ValidFileMatcher): Promise<string> {
...
export async function runMiddleware(request, ctx) {
  ...
  var fetchEvent = new NextFetchEvent({ page: url.pathname });
  var response;
