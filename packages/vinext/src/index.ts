    ...
    //    CDN is used instead.
    {
      ...
      transform: {
        ...
        async handler(code, id) {
          ...
          // Deduplicate utility imports to avoid duplicate specifiers in the
          // rewritten import statement (e.g. multiple imports of createFontLoader)
          const utilityImportsSet = new Set<string>();
          ...
          for (const raw of rawSpecifiers) {
            ...
            if (utilityExports.has(imported)) {
              utilityImportsSet.add(raw);
            } else {
              fontImports.push({ imported, local });
            }
          }

          const utilityImports = Array.from(utilityImportsSet);
          ...
          if ((this as any)._isBuild) {
            ...
            // NOTE: This regex runs on the original code string (not s.toString())
            // because MagicString tracks overwrites by original indices.
